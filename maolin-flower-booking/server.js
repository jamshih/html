...
const services = new Set(['花束／花禮預訂','開幕祝賀花禮','盆花／蘭花需求','追思／致意花禮','其他花禮需求']);
const fulfillment = new Set(['到店取花','外送需求','先諮詢']);
const times = new Set(['09:30','11:00','13:00','14:30','16:00','17:30','19:00']);
...
async function sendLine(row){
  ...
  const text=[
    ...
    row.delivery?`送達資訊：${row.delivery}`:null,
...
app.post('/api/book',async(req,res)=>{
  ...
  const row={id:`ML-${Date.now().toString(36).toUpperCase()}`,name,phone,email,service,fulfillment:fulfill,date,time,budget,delivery,note,createdAt:new Date().toISOString()};
...
app.listen(port,()=>console.log(`Maolin flower booking listening on ${port}`));
