...
const services = new Set(['鮮花花束','蘭花／盆花','高架花籃','婚禮／會場佈置','乾燥花／永生花','氣球佈置','其他花禮需求']);
const fulfillment = new Set(['到店取花','外送需求','先諮詢']);
const times = new Set(['08:30','10:00','11:30','13:00','14:30','16:00','17:30','19:00']);

function ensureStore(){fs.mkdirSync(dataDir,{recursive:true});if(!fs.existsSync(dataFile))fs.writeFileSync(dataFile,'[]','utf8');}
function readRows(){ensureStore();try{const value=JSON.parse(fs.readFileSync(dataFile,'utf8'));return Array.isArray(value)?value:[]}catch{return[]}}
...
function validDate(s){if(!/^\d{4}-\d{2}-\d{2}$/.test(s))return false;const d=new Date(`${s}T00:00:00+08:00`);return !Number.isNaN(d.getTime())&&s>=todayTW()}
...
async function sendLine(row){
  ...
  const text=[
    ...
    row.recipient?`收花／活動資訊：${row.recipient}`:null,
...
app.get('/api/health',(_req,res)=>res.json({ok:true,service:'sansan-flower-booking'}));
...
app.post('/api/book',async(req,res)=>{
  const name=clean(req.body.name,50),phone=clean(req.body.phone,30),email=clean(req.body.email,120),service=clean(req.body.service,40),fulfill=clean(req.body.fulfillment,20),date=clean(req.body.date,10),time=clean(req.body.time,5),budget=clean(req.body.budget,30),recipient=clean(req.body.recipient,140),note=clean(req.body.note,400);
  ...
  if(!validDate(date)||!times.has(time))return res.status(400).json({ok:false,error:'invalid_slot'});
  const rows=readRows();
  if(rows.filter(r=>r.date===date&&r.time===time).length>=3)return res.status(409).json({ok:false,error:'slot_full'});
  const row={id:`SS-${Date.now().toString(36).toUpperCase()}`,name,phone,email,service,fulfillment:fulfill,date,time,budget,recipient,note,createdAt:new Date().toISOString()};
...
app.listen(port,()=>console.log(`SanSan flower booking listening on ${port}`));
