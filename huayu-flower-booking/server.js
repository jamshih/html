...
const services = new Set(['花束預訂','盆花／蘭花','開幕花籃','婚禮／會場花藝','其他花禮需求']);
const fulfillment = new Set(['到店取花','外送需求','先諮詢']);
const times = new Set(['10:00','11:30','13:00','14:30','16:00','17:30','19:00']);
...
app.get('/api/health',(_req,res)=>res.json({ok:true,service:'huayu-flower-booking'}));
...
app.post('/api/book',async(req,res)=>{
  ...
  const row={id:`HY-${Date.now().toString(36).toUpperCase()}`,name,phone,email,service,fulfillment:fulfill,date,time,budget,note,createdAt:new Date().toISOString()};
...
app.listen(port,()=>console.log(`Huayu flower booking listening on ${port}`));
