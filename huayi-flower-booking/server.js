...
const services = new Set(['花束／節慶花禮','蘭花／盆花','開幕花籃','婚禮／會場佈置','氣球／其他佈置']);
const fulfillment = new Set(['到店取花','外送需求','先諮詢']);
const times = new Set(['09:30','11:00','12:30','14:00','15:30','17:00','18:30']);
...
app.get('/api/health',(_req,res)=>res.json({ok:true,service:'huayi-flower-booking'}));
...
app.post('/api/book',async(req,res)=>{
  ...
  const row={id:`HYF-${Date.now().toString(36).toUpperCase()}`,name,phone,email,service,fulfillment:fulfill,date,time,budget,note,createdAt:new Date().toISOString()};
...
app.listen(port,()=>console.log(`Huayi flower booking listening on ${port}`));
