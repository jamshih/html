...
const services = new Set(['傳情花束','蝴蝶蘭／盆景','開幕花籃','鮮花／氣球會場佈置','鮮花批發／大量需求','其他需求']);
const fulfillment = new Set(['到店取花','外送／宅配','先諮詢']);
const times = new Set(['09:00','11:00','13:00','15:00','17:00','19:00','21:00']);
...
function isValidDate(s){if(!/^\d{4}-\d{2}-\d{2}$/.test(s))return false;const d=new Date(`${s}T00:00:00+08:00`);return !Number.isNaN(d.getTime())&&s>=todayTW()}
...
app.get('/api/health',(_req,res)=>res.json({ok:true,service:'shanghao-flower-booking'}));
app.get('/api/availability',(req,res)=>{
  ...
  res.json({ok:true,date,available:[...times].filter(t=>(counts[t]||0)<4)});
});
app.post('/api/book',async(req,res)=>{
  ...
  const rows=readRows();
  if(rows.filter(r=>r.date===date&&r.time===time).length>=4)return res.status(409).json({ok:false,error:'slot_full'});
  const row={id:`SH-${Date.now().toString(36).toUpperCase()}`,name,phone,email,service,fulfillment:fulfill,date,time,budget,note,createdAt:new Date().toISOString()};
...
app.listen(port,()=>console.log(`Shanghao flower booking listening on ${port}`));
