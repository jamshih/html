...
const allowedServices = new Set(['男士剪髮', '染髮', '燙髮', '修邊／修容／修鬍', '頭皮護理', '其他需求']);
...
function isValidDate(dateString) {
  ...
  if (isMonday(dateString)) return false;
  return true;
}
...
app.get('/api/health', (_req, res) => {
  res.json({ ok: true, service: '73-barber-booking' });
});

app.get('/api/availability', (req, res) => {
  const date = clean(req.query.date, 10);
  if (!isValidDate(date)) {
    return res.status(400).json({ ok: false, error: isMonday(date) ? 'closed_monday' : 'invalid_date' });
  }
...
app.post('/api/book', async (req, res) => {
  ...
  if (!isValidDate(date)) return res.status(400).json({ ok: false, error: isMonday(date) ? 'closed_monday' : 'invalid_date' });
  ...
  const booking = {
    id: `73-${Date.now().toString(36).toUpperCase()}`,
  ...
  res.status(201).json({
    ok: true,
    booking: { id: booking.id, service: booking.service, date: booking.date, time: booking.time },
...
app.listen(port, () => {
  console.log(`73 Barber booking service listening on ${port}`);
});
