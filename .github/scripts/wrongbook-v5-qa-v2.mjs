...
async function backendQa(){
  const publicConfig=fs.readFileSync('wrongbook-v2/mod-01.js','utf8');
  const key=publicConfig.match(/SUPABASE_PUBLISHABLE_KEY='([^']+)'/)?.[1],root='https://rfpkznuntzxfoeeavwwf.supabase.co/functions/v1',out={health:{},errors:{},valid:{},providerSkipped:!RUN_PROVIDER};
  if(!key){fail('PUBLIC_CONFIG_FAIL','Supabase publishable key missing from app config');fs.writeFileSync(`${OUT}/backend.json`,JSON.stringify(out,null,2));return}
  const expectedHealth={
    'wrongbook-ai':{path:'/wrongbook-ai/health',model:'gemini-3.1-flash-lite',transport:'generateContent-native-schema',release:'v3-learning-objects-regions'},
    'wrongbook-sheet-ai':{path:'/wrongbook-sheet-ai',model:'gemini-3.1-flash-lite',transport:'generateContent-native-semantic-plus-spatial',version:'6-native-spatial-boxes'},
    'wrongbook-guide-ai':{path:'/wrongbook-guide-ai',model:'gemini-3.1-flash-lite',transport:'generateContent-native-schema',profile:'compact-grounded',version:'guide-v3-compact-grounded'}
  };
  for(const [slug,expect] of Object.entries(expectedHealth)){const r=await fetch(root+expect.path),j=await r.json();out.health[slug]={status:r.status,body:j};const ok=r.ok&&j?.ok===true&&j?.configured===true&&Object.entries(expect).filter(([k])=>k!=='path').every(([k,v])=>j?.[k]===v);if(!ok)fail('HEALTH_FAIL',{slug,j,expect})}
