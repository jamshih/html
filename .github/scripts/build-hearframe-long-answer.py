...
def speed_bounds(n):
    # Do not noticeably warp a single word. Longer phrase/sentence clips can absorb
    # slightly more tempo correction while keeping pitch stable with ffmpeg atempo.
...
if set(single_word_ids) != set(refined_map):
    # Extra entries are allowed only if they also belong to this answer; otherwise the
    # precision report and render plan have drifted apart.
...
speeds=[]
for i,(rate,n,role) in enumerate(zip(raw_wps,counts,ROLES)):
    lo,hi=speed_bounds(n)
    desired=(target_wps/rate) if rate>0 else 1.0
    speed=clamp(desired,lo,hi)
    # Avoid tiny encoder-only changes that no listener will notice.
    if abs(speed-1.0)<.012: speed=1.0
    # A conclusion benefits from a slightly more settled cadence, but stay inside
    # the same conservative bounds.
...
for i,(g,speed) in enumerate(zip(gains,speeds)):
    filters.append(f'[{i}:v]settb=AVTB,setpts=(PTS-STARTPTS)/{speed:.4f}[v{i}]')
...
report={
 ...
 'duration':float(probe['format']['duration']),'fullDecode':'pass','corpusVersionAfterBuild':corpus.get('version'),'alignment':corpus.get('alignment'),'probe':probe,
 'pass':True
}
