...
def extract_pair(ref,win,slot):
    ...
    if not got:return [],'frame_decode_failed:'+(p.stderr[-220:] if p.stderr else '')
...
def metrics(path):
    ...
    faces=face_detector().detectMultiScale(g,1.1,5,minSize=(30,30))
...
def audit(ref):
    ...
    if HARD_TITLE.search(title) or NEWS_TITLE.search(title):
        return {'referenceId':ref['referenceId'],'title':title,'pageUrl':ref.get('pageUrl'),'sourceUrl':ref.get('sourceUrl'),'sourceKind':ref.get('sourceKind'),
                'machineStatus':'REJECT','machineReasons':['hard_metadata_reject_before_positive_visual_review'],'samples':[],'sampleErrors':[]}
...
def make_sheets(rows):
    ...
    for pi in range(0,len(evidence),10):
        ...
        for ri,r in enumerate(block):
            y=ri*190
            cv2.putText(canvas,f"{r['referenceId']} {r['machineStatus']} {(r['title'] or '')[:84]}",(8,y+17),cv2.FONT_HERSHEY_SIMPLEX,.42,(255,255,255),1,cv2.LINE_AA)
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=8) as ex:
        ...
        for n,f in enumerate(as_completed(fut),1):
            try:r=f.result()
                ...
                rid=fut[f];r={'referenceId':rid,'title':'','pageUrl':None,'sourceUrl':None,'sourceKind':None,'machineStatus':'REJECT','machineReasons':['audit_worker_exception'], 'sampleErrors':[{'error':repr(exc)}],'samples':[]}
    ...
    for r in rows:
        ...
        counts[r['status']]+=1
    out={'version':'hearframe-m2-full-visual-audit-v2','sourceSet':'exact 131 counted English/alignment-ready refs from production-stitch-index-report',
