...
const services = new Set(['花束預訂','高架花籃','盆花／蘭花','追思花禮','其他花禮需求']);
const fulfillment = new Set(['到店取花','外送需求','先諮詢']);
const times = new Set(['08:00','10:00','12:00','14:00','16:00','18:00']);
...
async function sendLine(row){
  ...
  const text=[
    ...
    row.destination?`送達／活動資訊：${row.destination}`:null,
...
app.get('/api/health',(_req,res)=>res.json({ok:true,service:'liangyi-flower-booking'}));
...
app.post('/api/book',async(req,res)=>{
  ...
  const row={id:`LY-${Date.now().toString(36).toUpperCase()}`,name,phone,email,service,fulfillment:fulfill,date,time,budget,destination,note,createdAt:new Date().toISOString()};
...
app.listen(port,()=>console.log(`Liangyi flower booking listening on ${port}`));
