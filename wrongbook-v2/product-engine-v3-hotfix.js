...
// Legacy bind() only attached the first toggleMenu element (the top-bar button), which left
// the fixed bottom 「更多」 button dead on phones. Bind every mobile More button explicitly.
const v3HotfixBind=bind;
bind=function(){
  v3HotfixBind();
  document.querySelectorAll('.mobile-nav [data-action="toggleMenu"]').forEach(el=>{
    el.onclick=e=>{e.stopPropagation();state.mobileMenu=!state.mobileMenu;save();render()};
  });
};

// Rebind once so handlers capture the compatibility implementations and mobile fix above.
render();