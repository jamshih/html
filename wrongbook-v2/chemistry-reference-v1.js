...
function chemPaper(page,mode){const st=chemPageStats(page);return `<div class="chem-paper" data-chem-paper="${page.id}"><div class="chem-reference-banner"><span>脈 絡 整 合 ${page.number}・填 空 練 習</span></div><div class="chem-start"><i></i><span>從這裡出發</span></div><div class="chem-title-row"><i></i><h1>${chemEsc(page.title)}</h1><i></i></div>${page.clusters.map(c=>chemClusterHtml(page,c,mode)).join('')}${chemConnectorLayer()}<footer><span>＊ 掌握脈絡・填空鞏固・用圖像理解化學 ＊</span><small>${st.questions} 題 · ${st.blanks} 格 · ${st.figures} 個教學圖</small></footer></div>`}
...
let chemZoom=1;
function chemApplyZoom(next){chemZoom=Math.max(.45,Math.min(1.6,next));const stage=document.querySelector('[data-chem-stage]');if(stage){stage.style.transform=`scale(${chemZoom})`;stage.style.transformOrigin='top left';stage.parentElement.style.setProperty('--chem-zoom',chemZoom)}}
function chemFit(){const vp=document.querySelector('[data-chem-viewport]');if(!vp)return;chemApplyZoom(Math.min(1,(vp.clientWidth-20)/1000))}
function chemBind(){
