...
(function(){
  const VERSION='2026-08-18-paper-overlay-sticker-v10e';
  ...
  function limits(el,host){const hr=host.getBoundingClientRect(),er=el.getBoundingClientRect();return{hostW:Math.max(1,hr.width),hostH:Math.max(1,hr.height),maxX:Math.max(0,hr.width-er.width),maxY:Math.max(0,hr.height-er.height)}}
  function clampPosition(left,top,maxX,maxY){return{left:clamp(left,0,maxX),top:clamp(top,0,maxY)}}
  function persist(el,host){const {hostW,hostH}=limits(el,host),left=parseFloat(el.style.left)||0,top=parseFloat(el.style.top)||0;const store=readStore();store[stickerKey()]={x:clamp(left/hostW,0,1),y:clamp(top/hostH,0,1)};return writeStore(store)}
  function restore(el,host){const saved=readStore()[stickerKey()];if(!saved)return false;const {hostW,hostH,maxX,maxY}=limits(el,host),pos=clampPosition(Number(saved.x||0)*hostW,Number(saved.y||0)*hostH,maxX,maxY),left=Math.round(pos.left),top=Math.round(pos.top);const currentLeft=Math.round(parseFloat(el.style.left)||0),currentTop=Math.round(parseFloat(el.style.top)||0);if(currentLeft!==left)el.style.left=`${left}px`;if(currentTop!==top)el.style.top=`${top}px`;if(el.style.visibility!=='visible')el.style.visibility='visible';el.dataset.v10UserPositioned='1';return true}
  ...
  function bind(el){if(!el||el.dataset.v10StickerBound==='1')return;el.dataset.v10StickerBound='1';el.setAttribute('aria-roledescription','可拖曳 AI 圖解貼紙');const host=el.closest('.v9-paper-ai-layer');if(!host)return;requestAnimationFrame(()=>{if(!restore(el,host))persist(el,host)});let drag=null;el.addEventListener('pointerdown',e=>{if(e.button!=null&&e.button!==0)return;if(e.target.closest('button,a,input,textarea,select'))return;const r=el.getBoundingClientRect(),hr=host.getBoundingClientRect();drag={id:e.pointerId,startX:e.clientX,startY:e.clientY,left:r.left-hr.left,top:r.top-hr.top,moved:false};el.dataset.v10Dragging='1';try{el.setPointerCapture(e.pointerId)}catch{}e.preventDefault();e.stopPropagation()},true);el.addEventListener('pointermove',e=>{if(!drag||drag.id!==e.pointerId)return;const {maxX,maxY}=limits(el,host),dx=e.clientX-drag.startX,dy=e.clientY-drag.startY,pos=clampPosition(drag.left+dx,drag.top+dy,maxX,maxY);el.style.left=`${Math.round(pos.left)}px`;el.style.top=`${Math.round(pos.top)}px`;el.style.visibility='visible';if(Math.abs(dx)>2||Math.abs(dy)>2){drag.moved=true;el.dataset.v10UserPositioned='1'}e.preventDefault();e.stopPropagation()},true);const finish=e=>{if(!drag||drag.id!==e.pointerId)return;try{el.releasePointerCapture(e.pointerId)}catch{}const moved=drag.moved;drag=null;delete el.dataset.v10Dragging;if(moved)persist(el,host);e.preventDefault();e.stopPropagation()};el.addEventListener('pointerup',finish,true);el.addEventListener('pointercancel',finish,true)}
