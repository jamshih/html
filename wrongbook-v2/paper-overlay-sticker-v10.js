...
(function(){
  ...
  style.textContent=`
    ...
    .v9-sheet-ai-card::after{
      content:'⠿';
      position:absolute;
      top:7px;
      right:9px;
      z-index:4;
      color:color-mix(in srgb,var(--ink,#2B2D29) 48%,transparent);
      font-size:13px;
      line-height:1;
      pointer-events:none;
      opacity:.72;
  ...
  function persist(el,host){
    const {maxX,maxY}=limits(el,host),left=parseFloat(el.style.left)||0,top=parseFloat(el.style.top)||0;
    const store=readStore();store[stickerKey(el)]={x:maxX?clamp(left/maxX,0,1):0,y:maxY?clamp(top/maxY,0,1):0};return writeStore(store)
  }
  ...
  function bind(el){
    if(!el||el.dataset.v10StickerBound==='1')return;
    el.dataset.v10StickerBound='1';el.setAttribute('aria-roledescription','可拖曳 AI 圖解貼紙');
    const host=el.closest('.v9-paper-ai-layer');if(!host)return;
    requestAnimationFrame(()=>restore(el,host));
    ...
    el.addEventListener('pointermove',e=>{
      ...
      const {maxX,maxY}=limits(el,host),dx=e.clientX-drag.startX,dy=e.clientY-drag.startY,pos=clampPosition(drag.left+dx,drag.top+dy,maxX,maxY);
      el.style.left=`${Math.round(pos.left)}px`;el.style.top=`${Math.round(pos.top)}px`;el.style.visibility='visible';
      if(Math.abs(dx)>2||Math.abs(dy)>2){drag.moved=true;el.dataset.v10UserPositioned='1'}
      e.preventDefault();e.stopPropagation();
    },true);
    const finish=e=>{
      if(!drag||drag.id!==e.pointerId)return;
  ...
  window.runWrongbookAiStickerQA=function(){
    apply();const el=card(),host=layer(),draw=paper()?.querySelector('#drawCanvas'),toolbar=paper()?.querySelector('.paper-toolbar');
    if(!el||!host)return{pass:false,reason:'sticker-not-mounted',version:VERSION};
    const cs=getComputedStyle(el),hs=getComputedStyle(host),ds=draw?getComputedStyle(draw):null,ts=toolbar?getComputedStyle(toolbar):null;
