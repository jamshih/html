// Wrong Book V16d — single canonical compact tutor navigator.
// Owns visual shape + persistence + in-place paging. Legacy navigators are removed and never allowed to flash back in.
(function(){
  const VERSION='2026-08-18-tutor-nav-visual-v16d-canonical';
  ...
  function removeLegacy(){document.querySelectorAll('.v5-tutor-history-nav,.v7-tutor-step-nav,.v5-tutor-step-nav').forEach(el=>el.remove())}
  function ensureCanonical(){
    ...
    if(dock)dock.classList.add('v16-canonical-frame','v15-tutor-frame-stable','v17-tutor-frame-fixed');
    if(stage)stage.classList.add('v16-stable-stage');
    if(!stage)return null;
    const total=Array.isArray(s?.stages)?s.stages.length:0;
    let navs=[...stage.querySelectorAll(':scope > .v14-tutor-nav')];
    if(total<=1){navs.forEach(el=>el.remove());return null}
    let nav=navs.find(el=>el.classList.contains('v16-canonical-nav'))||null;
    if(!nav){const holder=document.createElement('div');holder.innerHTML=navMarkup(s);nav=holder.firstElementChild;const old=navs[0];if(old)old.replaceWith(nav);else stage.insertBefore(nav,stage.querySelector(':scope > .v5-tutor-actions')||null)}
    [...stage.querySelectorAll(':scope > .v14-tutor-nav')].forEach(el=>{if(el!==nav)el.remove()});
    nav.classList.add('v15-tutor-nav','v16-canonical-nav');
    const index=Math.max(0,Math.min(total-1,Number(s.activeIndex)||0)),prev=nav.querySelector('[data-v15-tutor-prev]'),next=nav.querySelector('[data-v15-tutor-next]'),count=nav.querySelector('.v14-tutor-nav-count');
  ...
  function updateStageDom(p,s,guide){
    ...
    const stage=s.stages[s.activeIndex],head=stageEl.querySelector(':scope > .v5-tutor-stage-head'),headLabel=head?.querySelector('span');if(headLabel&&typeof v5TutorStageLabel==='function')headLabel.textContent=v5TutorStageLabel(stage,s.activeIndex,s.stages.length);
    let prompt=stageEl.querySelector(':scope > p');if(!prompt){prompt=document.createElement('p');stageEl.insertBefore(prompt,stageEl.querySelector(':scope > .v5-tutor-actions')||null)}prompt.textContent=stage?.promptToStudent||stage?.goal||'';prompt.scrollTop=0;
  ...
  let queued=false;function queue(){if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;ensureCanonical()})}
  ...
  boot('./tutor-nav-paint-lock-v18.js?wb=20260818-1344','data-wb-tutor-nav-paint-lock-v18');boot('./ink-history-v4.js?wb=20260818-1344','data-wb-ink-history-v4-v16');

  window.wrongbookTutorNavVisualQA=function(){
    const nav=ensureCanonical(),s=currentSession(),expected=Boolean(s?.stages?.length>1),visibleLegacy=[...document.querySelectorAll('.v5-tutor-history-nav,.v7-tutor-step-nav,.v5-tutor-step-nav')].filter(el=>getComputedStyle(el).display!=='none'),allCanonical=[...document.querySelectorAll('.v16-canonical-nav')],dock=document.querySelector('.v5-tutor-dock');
    const buttons=nav?[...nav.querySelectorAll('button')]:[],count=nav?.querySelector('.v14-tutor-nav-count')?.textContent?.trim()||'',mobile=matchMedia('(max-width:700px)').matches,nr=nav?.getBoundingClientRect(),br=buttons[0]?.getBoundingClientRect(),eb=mobile?40:44,en=mobile?152:168,close=(a,b)=>Math.abs((a||0)-b)<=1;
    return{version:VERSION,pass:Boolean(visibleLegacy.length===0&&allCanonical.length<2&&(!expected||(nav&&buttons.length===2&&count.includes('/')&&close(nr?.width,en)&&close(br?.width,eb)))&&(!dock||getComputedStyle(dock).animationName==='none')),expectedNavigator:expected,canonicalNavigatorCount:allCanonical.length,visibleLegacyNavigatorCount:visibleLegacy.length,navigatorPresent:Boolean(nav),buttonCount:buttons.length,count,navigatorWidth:Math.round(nr?.width||0),buttonSize:Math.round(br?.width||0),singleNavigationOwner:true,selfHealingAfterRender:true,inPlacePagingNoRender:true,outerFrameAnimationSuppressed:!dock||getComputedStyle(dock).animationName==='none'};
  };
})();
