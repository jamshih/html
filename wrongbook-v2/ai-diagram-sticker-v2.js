...
(function(){
  ...
  const STORE='wrongbook:ai-diagram-sticker:v3';
  let topZ=2147482100;
  let active=null;
  ...
  style.textContent=`
    ...
    .wb-ai-sticker-v2 *:not(button):not(a):not(input):not(textarea):not(select):not(summary):not([contenteditable="true"]){
      cursor:grab!important;
    }
  ...
  function title(card){
    const node=[...card.querySelectorAll('h1,h2,h3,h4,strong,[class*="title" i]')]
      .find(el=>{const t=norm(el.textContent);return t&&t!==LABEL&&!t.includes(LABEL)&&t.length<100});
  ...
  function readSaved(card){
    try{
      ...
      return data&&data.floating===true&&Number.isFinite(data.left)&&Number.isFinite(data.top)?data:null;
    }catch{return null}
  }
  ...
  function save(card){
    ...
    try{localStorage.setItem(storageKey(card),JSON.stringify({floating:true,left:Math.round(r.left),top:Math.round(r.top),width:Math.round(r.width),z:Number(card.style.zIndex)||topZ}))}catch{}
  }

  function bringFront(card,z){
  ...
  function float(card,saved){
    ...
    if(!parent)return;
    const ph=placeholder(card,rect);
    parent.insertBefore(ph,card);
    card.__wbAiV2Anchor=ph;
    layer().appendChild(card);
    card.classList.add('wb-ai-is-floating');
  ...
  function keepVisible(card){
    ...
    const maxLeft=Math.max(INSET,innerWidth-r.width-INSET);
    const maxTop=Math.max(INSET,innerHeight-visibleHeight-INSET);
    card.style.left=`${Math.round(clamp(r.left,INSET,maxLeft))}px`;
    card.style.top=`${Math.round(clamp(r.top,INSET,maxTop))}px`;
  }
  ...
  function looksLikeCard(el,label){
    ...
    if(r.width>Math.max(innerWidth*.99,1500)||r.height>Math.max(innerHeight*2,1800))return false;
    const cs=getComputedStyle(el);
    const radius=parseFloat(cs.borderTopLeftRadius)||0;
    const visual=el.querySelector('svg,canvas,img,picture,video,[class*="diagram" i],[class*="figure" i],[class*="visual" i]');
    return Boolean(visual)&&(radius>=6||parseFloat(cs.borderTopWidth)>0||cs.boxShadow!=='none'||/card|panel|diagram|figure|visual/i.test(String(el.className||'')));
  }

  function cardForLabel(label){
    // Prefer an explicit renderer contract when available.
  ...
  function upgrade(card){
    ...
    card.classList.add('wb-ai-sticker-v2');
    card.setAttribute('data-ai-sticker-draggable','true');
    const saved=readSaved(card);
    if(saved)requestAnimationFrame(()=>{if(card.isConnected){float(card,saved);save(card)}});
  }
  ...
  function scan(root=document.getElementById('app')||document.body){
    ...
    for(const card of explicit)upgrade(card);
    for(const label of labelElements(root)){
      const card=cardForLabel(label);
  ...
  function cardFromPointer(target){
    ...
    const ready=target.closest('.wb-ai-sticker-v2,[data-ai-diagram-card]');
    if(ready){upgrade(ready);return ready}
    // Last-resort live lookup: climb the pointer ancestry and accept the smallest visual card
    // whose visible text contains the AI label. This makes dragging work even before observer scan.
    let el=target;
    for(let i=0;i<12&&el&&el!==document.body;i++,el=el.parentElement){
      if(!norm(el.textContent).includes(LABEL))continue;
      const r=el.getBoundingClientRect();
      const visual=el.querySelector?.('svg,canvas,img,picture,[class*="diagram" i],[class*="figure" i],[class*="visual" i]');
      if(r.width>=240&&r.height>=150&&visual){upgrade(el);return el}
    }
    return null;
  }
  ...
  function start(card,e){
    ...
    const r=card.getBoundingClientRect();
    bringFront(card);
    active={card,id:e.pointerId,startX:e.clientX,startY:e.clientY,left:r.left,top:r.top};
    card.classList.add('wb-ai-is-dragging');
    try{card.setPointerCapture(e.pointerId)}catch{}
    // Capture-phase interception prevents legacy handlers from stealing the same drag.
    e.preventDefault();
    e.stopPropagation();
  }
  ...
  function move(e){
    ...
    const visibleHeight=Math.min(r.height,Math.max(80,innerHeight-INSET*2));
    const maxTop=Math.max(INSET,innerHeight-visibleHeight-INSET);
    d.card.style.left=`${Math.round(clamp(d.left+(e.clientX-d.startX),INSET,maxLeft))}px`;
    d.card.style.top=`${Math.round(clamp(d.top+(e.clientY-d.startY),INSET,maxTop))}px`;
    e.preventDefault();
  }
  ...
  function end(e){
    ...
    if(!d||e.pointerId!==d.id)return;
    active=null;
    d.card.classList.remove('wb-ai-is-dragging');
    try{d.card.releasePointerCapture(e.pointerId)}catch{}
    keepVisible(d.card);
    save(d.card);
  }

  // The important V2 change: pointerdown is delegated at document CAPTURE phase and the WHOLE
  // AI card is the drag surface. It cannot fail just because a generated header has a new DOM shape.
  document.addEventListener('pointerdown',e=>{
    const card=cardFromPointer(e.target);
    if(card)start(card,e);
  },true);
  ...
  // Small runtime QA surface for browser automation / console inspection.
  window.__wrongbookAiStickerV2QA=function(){
    const cards=[...document.querySelectorAll('.wb-ai-sticker-v2')];
    return {loaded:true,cards:cards.length,draggable:cards.filter(c=>c.dataset.aiStickerDraggable==='true').length,floating:cards.filter(c=>c.classList.contains('wb-ai-is-floating')).length,layer:Boolean(document.getElementById('wb-ai-sticker-v2-layer'))};
  };
})();