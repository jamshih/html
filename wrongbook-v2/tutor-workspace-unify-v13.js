// Wrong Book V13 — one tutor UI, one compact step navigator, and more room to write.
// Scope intentionally limited to the problem worksheet + tutor surface.
(function(){
  const VERSION='2026-08-18-tutor-workspace-unify-v13';
  ...
  if(!document.getElementById(STYLE_ID)){
    ...
    style.textContent=`
      ...
      .v5-tutor-step-track{gap:5px!important}
      .v5-tutor-step-dot{width:8px!important;height:8px!important}
      .v5-tutor-step-count{font-size:11px!important;margin-left:4px!important}
  ...
  window.wrongbookTutorWorkspaceUnifyQA=function(){normalizeTutorDom();const paper=document.querySelector('.v3-paper'),dock=document.querySelector('.v5-tutor-dock');const navs=dock?[...dock.querySelectorAll('.v5-tutor-step-nav,.v5-tutor-history-nav,.v7-tutor-step-nav')].filter(el=>getComputedStyle(el).display!=='none'):[];const canonical=dock?.querySelector('.v5-tutor-step-nav')||null,paperHeight=paper?parseFloat(getComputedStyle(paper).minHeight)||0:0,navWidth=canonical?canonical.getBoundingClientRect().width:0,onPaper=document.querySelector('[data-action="aiOnPaper"]'),session=currentSession();return{version:VERSION,pass:Boolean((!dock||navs.length<=1)&&(!canonical||navWidth<=432)&&(!paper||paperHeight>=680)&&(!onPaper||onPaper.dataset.tutorOwner==='v13')),visibleTutorNavCount:navs.length,canonicalNavPresent:Boolean(canonical),canonicalNavWidth:Math.round(navWidth),paperMinHeight:paperHeight,onPaperOwnedByUnifiedTutor:onPaper?.dataset.tutorOwner==='v13'||false,legacyHistoryNavPresent:Boolean(document.querySelector('.v5-tutor-history-nav')),legacyV7NavPresent:Boolean(document.querySelector('.v7-tutor-step-nav')),sessionMode:session?.mode||null,textStageAvailable:Boolean(session?.stages?.[session?.activeIndex]?.promptToStudent)}};
})();
