...
(function(){
 const esc=s=>typeof v4RefEsc==='function'?v4RefEsc(String(s)):String(s);
 function chapterFor(page){const no={242:1,243:1,244:2,245:2,246:3,247:3,248:4,249:4,250:5,251:5,252:6,253:6}[page];return (window.EARTH_REFERENCE_MAPS||[]).find(x=>x.number===no);}
 ...
 function group(id,kind,inner,extra=''){
   return `<div class="v9-group ${extra}" data-source-object="${id}" data-source-role="${kind}" data-parent-id="${window.SOURCE_HIERARCHY_V9?.[252]?.nodes?.[id]?.parentId||window.SOURCE_HIERARCHY_V9?.[253]?.nodes?.[id]?.parentId||''}" data-container-kind="${window.SOURCE_HIERARCHY_V9?.[252]?.nodes?.[id]?.containerKind||window.SOURCE_HIERARCHY_V9?.[253]?.nodes?.[id]?.containerKind||'none'}">${inner}</div>`;
 }
 ...
 const ENSO=`<svg class="v9-p252-enso-figure" data-source-role="graph" data-graph-kind="enso" viewBox="0 0 335 255" aria-label="聖嬰年赤道太平洋剖面"><defs><marker id="v9e252" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="5" markerHeight="5" orient="auto"><path d="M0 0L10 5L0 10Z" fill="#626262"/></marker></defs><rect x="38" y="62" width="272" height="155" fill="#f7f3e9" stroke="#9b958a"/><path d="M42 150Q124 108 200 115Q258 120 306 137V215H42Z" fill="#4e88b6"/><path d="M42 125Q120 88 200 98Q260 105 306 120V145Q244 128 197 125Q116 116 42 145Z" fill="#e48158"/><path d="M47 137Q126 111 205 119Q263 124 303 134" fill="none" stroke="#efb24a" stroke-width="10"/><g stroke="#5c5c58" stroke-width="4" marker-end="url(#v9e252)"><path d="M105 122V91"/><path d="M147 115V84"/><path d="M190 118V88"/><path d="M253 80V111"/></g><text x="126" y="80" font-size="11">對流上升</text><text x="210" y="154" font-size="11">赤道東風減弱</text><text x="48" y="237" font-size="11">西太平洋</text><text x="246" y="237" font-size="11">東太平洋</text></svg>`;
 const TEMP=`<svg class="v9-p252-temp-figure" data-source-role="graph" data-graph-kind="temperature-depth" viewBox="0 0 185 190" aria-label="海水溫度垂直分層"><path d="M28 18H172M28 18V176" stroke="#555" stroke-width="1.5"/><path d="M41 24C117 24 139 31 131 50C117 78 79 85 68 111C62 130 61 152 61 174" fill="none" stroke="#bd5450" stroke-width="3"/><path d="M28 67H172M28 120H172" stroke="#8ba0af" stroke-width="1" stroke-dasharray="4 4"/><text x="86" y="12" font-size="9">溫度（°C）</text><text x="9" y="106" font-size="9" transform="rotate(-90 9 106)">深度（公里）</text></svg>`;
 const SAL=`<svg class="v9-p252-sal-figure" data-source-role="graph" data-graph-kind="surface-salinity" viewBox="0 0 290 170" aria-label="表面鹽度與蒸發降水緯度分布"><rect x="25" y="10" width="245" height="125" fill="#fff" stroke="#aaa"/><g stroke="#ddd"><path d="M25 40H270M25 70H270M25 100H270M75 10V135M125 10V135M175 10V135M225 10V135"/></g><path d="M28 94C72 117 103 83 145 69C179 58 216 63 268 82" fill="none" stroke="#4e7eaa" stroke-width="2.5"/><path d="M28 107C65 72 104 45 145 74C184 101 219 80 268 50" fill="none" stroke="#6da268" stroke-width="2.2"/><path d="M28 65C72 54 106 107 145 99C185 92 219 43 268 73" fill="none" stroke="#d28659" stroke-width="2.2"/><text x="188" y="45" font-size="9" fill="#6da268">蒸發量</text><text x="192" y="102" font-size="9" fill="#d28659">降水量</text><text x="80" y="91" font-size="9" fill="#4e7eaa">鹽度</text><text x="27" y="151" font-size="9">90°</text><text x="140" y="151" font-size="9">0°</text><text x="252" y="151" font-size="9">90°</text></svg>`;
 function page252(ch,mode){
   const ty=[1,2,3,4,5,6,7].map(n=>prompt(ch,252,n,mode,`q${n}`)).join('');
   const ens=group('p252-enso','graphic-container',`<div class="v9-ribbon purple small">聖嬰現象</div><div class="v9-pill purple">聖嬰年</div>${ENSO}${prompt(ch,252,17,mode,'q17')}${prompt(ch,252,19,mode,'q19')}${prompt(ch,252,18,mode,'q18')}`,'v9-p252-enso');
   ...
   const mixed=group('p252-mixed-upwelling','source-box',`${prompt(ch,252,11,mode,'q11')}${prompt(ch,252,16,mode,'q16')}`,'v9-p252-mixed');
   ...
   const ocean=`<div class="v9-ribbon blue ocean-title">海　洋</div><div class="v9-ribbon blue horizontal-title">水平變化</div><div class="v9-ribbon blue temp-title">海水溫度</div><div class="v9-ribbon blue vertical-title">垂直變化</div>${horiz}${group('p252-ocean-temp','group',mixed+temp+exp,'v9-p252-ocean-temp')}<div class="v9-ribbon blue salt-title">海水鹽類</div>${group('p252-salinity','group',sal,'v9-p252-salinity')}${group('p252-salinity-graph','protected-figure',SAL,'v9-p252-salgraph')}`;
 ...
 const PLATE=`<svg class="v9-p253-plate-graphic" data-source-role="figure" data-figure-kind="plate-climate" viewBox="0 0 755 92"><path d="M0 39Q110 20 220 34Q340 50 455 31Q600 12 755 32V92H0Z" fill="#b79a77"/><path d="M15 35Q90 21 165 31M285 37Q380 21 455 31M545 30Q635 14 720 29" fill="none" stroke="#87765d" stroke-width="2"/></svg>`;
 const CLOUD=`<svg class="v9-p253-cloud-graphic" viewBox="0 0 235 100" aria-hidden="true"><path d="M8 82C-4 60 15 43 34 48C43 24 70 23 84 43C103 20 135 28 139 50C166 34 193 45 196 66C220 61 237 76 226 93H10Z" fill="#efd0c1" stroke="#d69d82" stroke-width="1.5"/></svg>`;
 function p253BranchSvg(){return `<svg class="v9-p253-ocean-lines" data-source-role="connector" viewBox="0 0 780 675" aria-hidden="true"><g fill="none" stroke="#57955b" stroke-width="4" stroke-linecap="square" stroke-linejoin="miter"><path d="M34 85V630"/><path d="M34 85H72"/><path d="M250 106V215M250 120H278M250 167H278M250 213H278"/><path d="M34 280H82M82 280H106"/><path d="M34 385H82M82 365V462M82 365H111M82 410H111M82 460H111"/><path d="M335 383V501M335 402H357M335 449H357M335 496H357"/><path d="M34 535H104M104 535V666M104 565H127M104 596H127M104 627H127M104 660H127"/></g><g fill="#fff" stroke="#666" stroke-width="2"><circle cx="34" cy="85" r="6"/><circle cx="34" cy="280" r="6"/><circle cx="34" cy="385" r="6"/><circle cx="34" cy="535" r="6"/></g></svg>`}
 function page253(ch,mode){
   const climateLeft=group('p253-climate-left','group',`${prompt(ch,253,27,mode,'q27')}${prompt(ch,253,28,mode,'q28')}${group('p253-greenhouse-cloud','graphic-container',`${CLOUD}<div class="v9-cloud-label">溫室氣體包含</div>${prompt(ch,253,29,mode,'q29')}${prompt(ch,253,30,mode,'q30')}`,'v9-p253-cloud')}${prompt(ch,253,31,mode,'q31')}${prompt(ch,253,32,mode,'q32')}<div class="v9-greenhouse-effect">溫室效應</div>`,'v9-p253-climate-left');
 ...
 function annotate(html,page){
   ...
   for(const [id,node] of Object.entries(h.nodes)){if(id===h.rootId)continue; const matches=sec.querySelectorAll(`[data-source-object="${id}"]`);matches.forEach(el=>{el.dataset.parentId=node.parentId||h.rootId;el.dataset.containerKind=node.containerKind||'none';});}
   return t.innerHTML;
 }
 const prev=window.v5PageHtml;if(typeof prev!=='function')return;
 window.v5PageHtml=function(ch,sem,page,mode){
   if(page===252)return page252(ch,mode);
   if(page===253)return page253(ch,mode);
   return annotate(prev(ch,sem,page,mode),page);
 };
