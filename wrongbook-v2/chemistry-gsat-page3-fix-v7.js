...
(function chemistryGsatPage3FixV7(){
  ...
  const q11=mole?.questions?.find(q=>q.n===11),q18=calc?.questions?.find(q=>q.n===18),q20=calc?.questions?.find(q=>q.n===20),q21=calc?.questions?.find(q=>q.n===21);
  if(q11)q11.prompt='利用「質量 ↔ {{0}} ↔ 粒子數」轉換，可把反應式係數連到實際質量。';
  if(q18)q18.prompt='定量計算先把已知量轉成 {{0}}，再用反應式係數比求未知物。';
  if(q20)q20.prompt='反應物不符合係數比時，先耗盡並限制產物量的是 {{0}} 反應物。';
