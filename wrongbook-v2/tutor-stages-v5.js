...
const V5_TUTOR_VERSION='2026-08-14-tutor-stages-v5';
...
function v5TutorNextDirect(){const p=selectedProblem(),s=v5TutorSession(p);if(!p||!s)return;const next=s.activeIndex+1;if(next<s.stages.length){v5CancelGuidePlayback();const g=v5BuildStageGuide(p,s,next);save();render();if(g)setTimeout(()=>v3GuideReplay(),45)}else if(!s.stages[s.activeIndex]?.revealFinalAnswer)v5TutorCall('next',{mode:'direct'})}
...
function v5TutorControls(s,stage){
  ...
  if(s.mode==='direct'){const hasMore=s.activeIndex<s.stages.length-1;if(!hasMore&&stage?.revealFinalAnswer)return `<div class="v5-tutor-actions"><button class="primary-btn" disabled>詳解完成</button></div>`;return `<div class="v5-tutor-actions"><button class="primary-btn" data-v5-tutor-next>${hasMore?'下一步':'繼續下一步'}</button></div>`}
...
v3GuideMarkup=function(p){
  ...
  return `<canvas id="aiGuideCanvas" class="v3-guide-canvas ${hidden?'is-hidden':''}" aria-hidden="true"></canvas><div class="v3-guide-dock v5-tutor-dock" data-guide-dock><div class="v5-tutor-mode-switch" role="group" aria-label="AI 家教模式"><button class="${state.aiGuideMode==='instructive'?'active':''}" data-v5-tutor-mode="instructive">引導我解題</button><button class="${state.aiGuideMode==='direct'?'active':''}" data-v5-tutor-mode="direct">直接看詳解</button></div>${s?`<div class="v5-tutor-stage"><div class="v5-tutor-stage-head"><span>${v5TutorStageLabel(stage,s.activeIndex,s.stages.length)}</span>${diagnosis?.studentOnRightTrack===true?'<strong class="is-right">✓ 目前方向正確</strong>':diagnosis?.blindSpot?`<strong>${esc(diagnosis.blindSpot)}</strong>`:''}</div>${stage?.promptToStudent?`<p>${esc(stage.promptToStudent)}</p>`:''}${v5TutorControls(s,stage)}</div>`:`<div class="v5-tutor-empty"><div><strong>先看你已經寫到哪裡，再只帶下一步。</strong><span>預設不會把完整答案一次揭露。</span></div><button class="primary-btn" data-action="guideStart">開始引導</button></div>`}<div class="v5-guide-utility"><span data-guide-status>${guide?'這一步可以重播':'AI 筆跡會直接畫在原題上'}</span><span class="v3-guide-caption" data-guide-caption>${guide?.explanation?esc(guide.explanation.slice(0,88)):''}</span><div><button class="soft-btn" data-action="guideReplay" ${guide?'':'disabled'}>重播</button><button class="soft-btn" data-action="guideSpeed" ${guide?'':'disabled'}>${speed}×</button><button class="soft-btn" data-action="guideHide" ${guide?'':'disabled'}>${hidden?'顯示筆跡':'隱藏筆跡'}</button></div></div></div>`;
};
...
window.v5TutorStart=v5TutorStart;window.v5TutorEvaluate=v5TutorEvaluate;window.v5TutorHint=v5TutorHint;window.v5CancelGuidePlayback=v5CancelGuidePlayback;
