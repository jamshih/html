...
function v2Figures(ids,mode,layout,assetIds=[],title=''){const figs=(ids||[]).map(id=>chemFigure(id,mode)),assets=typeof mindmapApprovedAssetHtml==='function'?mindmapApprovedAssetHtml(assetIds,{className:'mindmap-asset-group chem-approved-assets',label:`${title}概念插圖`}):'';return `<div class="chem-v2-figures layout-${layout||'grid'} count-${figs.length} ${assets?'has-approved-assets':''}">${figs.join('')}${assets}</div>`}
chemClusterHtml=function(page,c,mode){if(page.id==='gsat-01')return oldCluster(page,c,mode);const layout=c.layout||((c.w>700)?'wide':'side'),assetIds=typeof mindmapApprovedAssetIds==='function'?mindmapApprovedAssetIds('chemistry',page.id,c.id):[];return `<section class="chem-cluster chem-v2-cluster chem-${c.id} layout-${layout}" style="left:${c.x}px;top:${c.y}px;width:${c.w}px;height:${c.h}px;--chem-accent:${page.theme}"><header><b>${c.number}</b><h3>${chemEsc(c.title)}</h3></header><div class="chem-v2-cluster-body"><div class="chem-v2-questions">${chemQuestions(page,c,mode)}</div>${v2Figures(c.figures,mode,layout,assetIds,c.title)}</div></section>`};
