...
(function(){
  const VERSION='2026-08-18-ocr-clean-question-display-v3.1-integrity';
  ...
  function tutorMarkup(p){
    try{return typeof v3GuideMarkup==='function'?v3GuideMarkup(p):''}catch{return''}
  }
  ...
  paperPanel=function(p){
    ...
    const prompt=String(p.problemText||p?.ocrResult?.questionText||state.scanOCR?.questionText||'').trim(),tutor=tutorMarkup(p);
    // v3-paper + paper-toolbar are semantic compatibility hooks for the workspace integrity layer.
    // Without them, that layer failed to recognize an OCR worksheet as a valid paper and replaced
    // .pf-paper-column.innerHTML on every MutationObserver pass, creating an infinite render storm.
    return `<section class="panel ocrq-panel"><div class="panel-head"><div><h3>題目</h3><span class="meta">完整 OCR 題幹 · 鄰題、頁邊與無用背景已排除</span></div>${qualityBadge(p)}</div><div class="ocrq-paper"><div class="ocrq-sheet v3-paper" id="paper"><article class="ocrq-prompt-card"><div class="ocrq-prompt-label">題目</div><div class="ocrq-prompt-text">${escH(prompt)}</div>${figs.length?`<div class="ocrq-inline-figures"><div class="ocrq-prompt-label">題目的圖</div>${figs.map((u,i)=>`<div class="ocrq-inline-figure" ${i?'style="margin-top:12px"':''}><img src="${u}" alt="題目的圖 ${i+1}"></div>`).join('')}</div>`:''}</article>${overlays}<canvas id="drawCanvas" class="canvas-layer"></canvas><div class="ocrq-file-note"><strong>清楚題目檔已建立</strong> · 文字用 OCR 結構重建；題圖保留原始高解析裁切。原始掃描只作核對。</div>${tutor}<div class="ocrq-toolbar paper-toolbar"><div class="toolset"><button class="tool active" data-tool="pen">✎</button><button class="tool" data-tool="eraser">⌫</button><button class="tool" data-action="undoInk">↶</button><button class="tool" data-action="clearInk">清除</button></div><div class="toolset"><button class="tool" data-action="aiOnPaper">✦ AI 在題目上提示</button></div></div></div></div>${warn.length?`<div class="ocrq-warnings">${warn.map(x=>`• ${escH(x)}`).join('<br>')}</div>`:''}${raw?`<details class="ocrq-source"><summary>原始掃描對照</summary><img src="${raw}" alt="原始題目掃描對照"></details>`:''}</section>`;
  };
  try{window.paperPanel=paperPanel}catch{}
  window.wrongbookOCRDisplayQA=()=>{
    const p=(()=>{try{return typeof selectedProblem==='function'?selectedProblem():null}catch{return null}})(),scan=Boolean(p&&String(p.id||'').startsWith('scan-')),tutorExpected=scan&&typeof v3GuideMarkup==='function',tutorDockPresent=Boolean(document.querySelector('.ocrq-sheet .v5-tutor-dock,.ocrq-sheet .v3-guide-dock')),sheet=document.querySelector('.ocrq-sheet'),toolbar=document.querySelector('.ocrq-toolbar');
    return{version:VERSION,cleanFilePrimary:true,fullPromptNativeText:true,problemFiguresInline:true,rawScanSecondary:true,cleanArtifactStored:Boolean(state.scanCleanFile||p?.cleanQuestionFile),figureCount:embeddedFigures(state.scanCleanFile||'').length,tutorMarkupOwned:true,tutorExpected,tutorDockPresent:!tutorExpected||tutorDockPresent,workspaceIntegrityCompatible:Boolean(!scan||(sheet?.classList.contains('v3-paper')&&toolbar?.classList.contains('paper-toolbar'))),pass:Boolean((!tutorExpected||tutorDockPresent)&&(!scan||(sheet?.classList.contains('v3-paper')&&toolbar?.classList.contains('paper-toolbar'))))};
  };
})();
