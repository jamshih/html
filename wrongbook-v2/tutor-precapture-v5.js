...
const V5_TUTOR_PRECAPTURE_VERSION='2026-08-17-tutor-precapture-v5b';
...
async function v5TutorTakePrecapture(kind){
  ...
  const snap={version:V5_TUTOR_PRECAPTURE_VERSION,problemId:p.id,kind:String(kind||''),capturedAt:new Date().toISOString(),workspace:v5TutorCaptureClone(workspace),zoom:v5TutorCaptureClone(zoom),pathCount:paths.length,pointCount,hadInk:Boolean(paths.length&&pointCount)};
  window.__v5TutorCapture=snap;
  window.__v5TutorCaptureLast={...snap,workspace:workspace?{mimeType:workspace.mimeType,width:workspace.width,height:workspace.height,hasPhoto:workspace.hasPhoto,base64Length:workspace.base64?.length||0}:null,zoom:zoom?{mimeType:zoom.mimeType,width:zoom.width,height:zoom.height,bounds:zoom.bounds,base64Length:zoom.base64?.length||0}:null};
  return snap;
}

function v5TutorPrecaptureInstall(){
  let ready=true;
  if(typeof v5TutorVisual==='function'&&!window.__v5TutorPrecaptureVisualWrapped){
    ...
    v5TutorVisual=function(){const p=typeof selectedProblem==='function'?selectedProblem():null,snap=window.__v5TutorCapture;if(snap?.workspace?.base64&&snap.problemId===p?.id)return Promise.resolve(v5TutorCaptureClone(snap.workspace));return baseTutorVisual.apply(this,arguments)};
  }else if(typeof v5TutorVisual!=='function')ready=false;
  ...
  if(typeof v5VisionHandwritingZoom==='function'&&!window.__v5TutorPrecaptureZoomWrapped){
    ...
    v5VisionHandwritingZoom=async function(){const p=typeof selectedProblem==='function'?selectedProblem():null,snap=window.__v5TutorCapture;if(snap?.zoom?.base64&&snap.problemId===p?.id)return v5TutorCaptureClone(snap.zoom);return baseVisionZoom.apply(this,arguments)};
  }else if(typeof v5VisionHandwritingZoom!=='function')ready=false;
  ...
  if(typeof v5TutorCall==='function'&&!window.__v5TutorPrecaptureCallWrapped){
    ...
    v5TutorCall=async function(kind,opts){
      ...
      finally{const snap=window.__v5TutorCapture;if(!p||snap?.problemId===p.id)window.__v5TutorCapture=null}
    };
  }else if(typeof v5TutorCall!=='function')ready=false;
  return ready;
}

(function v5TutorPrecaptureBootstrap(){
  if(v5TutorPrecaptureInstall())return;
  let tries=0;const timer=setInterval(()=>{tries++;if(v5TutorPrecaptureInstall()||tries>=320)clearInterval(timer)},25);
})();
