...
const huashanDataFile = path.join(dataDir, 'huashan-requests.json');
...
const huashanPublic = path.join(__dirname, '..', 'huashan-kiln-booking', 'public');

app.use(express.json({ limit: '100kb' }));
app.use(express.static(path.join(__dirname, 'public')));
app.use('/asahikari', express.static(asahikariPublic));
app.use('/linhanwei', express.static(linhanweiPublic));
app.use('/huashan', express.static(huashanPublic));
...
const huashanRequestTypes = new Set(['手拉胚／陶藝體驗','親子／團體體驗','學校／企業課程','作品／商品詢問','展覽／品牌合作','其他需求']);
...
async function sendHuashanLine(row){
  const text=[
    '🏺 華山窯｜新陶藝體驗／合作需求',
    `聯絡人：${row.name}`,
    `Email：${row.email}`,
    ...
    `內容：${row.note}`,
    `編號：${row.id}`,
    '※ 此為需求申請，實際日期、名額、內容與價格仍需華山窯確認。'
  ].filter(Boolean).join('\n');
  return pushLine(process.env.HUASHAN_LINE_CHANNEL_ACCESS_TOKEN,process.env.HUASHAN_LINE_OWNER_USER_ID,text,'Huashan LINE');
}

app.get('/api/health',(_req,res)=>res.json({ok:true,service:'fleetstreet-booking',asahikariMounted:true,linhanweiMounted:true,huashanMounted:true}));
...
app.get('/huashan/api/health',(_req,res)=>res.json({ok:true,service:'huashan-kiln-booking'}));
app.post('/huashan/api/request',async(req,res)=>{
  const name=clean(req.body.name,50);
  const email=clean(req.body.email,120);
  const phone=clean(req.body.phone,30);
  const requestType=clean(req.body.requestType,40);
  ...
  if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return res.status(400).json({ok:false,error:'invalid_email'});
  if(phone && !/^[0-9+()\-\s]{8,20}$/.test(phone)) return res.status(400).json({ok:false,error:'invalid_phone'});
  if(!huashanRequestTypes.has(requestType)) return res.status(400).json({ok:false,error:'invalid_request_type'});
  if(date && !isValidDate(date)) return res.status(400).json({ok:false,error:'invalid_date'});
  if(note.length<5) return res.status(400).json({ok:false,error:'note_required'});
  ...
  const row={id:`HSK-${Date.now().toString(36).toUpperCase()}`,name,email,phone,requestType,date:date||null,people,note,status:'pending_confirmation',createdAt:new Date().toISOString()};
  const rows=readRows(huashanDataFile); rows.push(row); writeRows(rows,huashanDataFile);

  let line={sent:false,reason:'not_configured'};
  try { line=await sendHuashanLine(row); } catch(e){ console.error(e.message); }
  res.status(201).json({ok:true,request:{id:row.id,requestType,status:row.status},lineNotification:line.sent,message:'需求已送出，待華山窯確認後才成立。'});
});

app.use((_req,res)=>res.sendFile(path.join(__dirname,'public','index.html')));
app.listen(port,'0.0.0.0',()=>console.log(`Fleet Street booking listening on ${port}; Asahikari mounted at /asahikari; LinHanWei mounted at /linhanwei; Huashan mounted at /huashan`));
